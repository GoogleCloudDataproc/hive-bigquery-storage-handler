...
public class HiveBigQueryMetaHook implements HiveMetaHook {
    ...
    public void preCreateTable(Table table) throws MetaException {
        ...
        */
        ...
        //Assign Hive databasename as Big Query Dataset name if not provided by user
        if (Strings.isNullOrEmpty(table.getParameters().get(HiveBigQueryConstants.DEFAULT_BIGQUERY_DATASET_KEY))) {
            table.getParameters().put(HiveBigQueryConstants.DEFAULT_BIGQUERY_DATASET_KEY, table.getDbName());
        }

        //Assign Hive table as Big Query Table name if not provided by user
        if (Strings.isNullOrEmpty(table.getParameters().get(HiveBigQueryConstants.DEFAULT_BIGQUERY_TABLE_KEY))) {
            table.getParameters().put(HiveBigQueryConstants.DEFAULT_BIGQUERY_TABLE_KEY, table.getTableName());
        }
